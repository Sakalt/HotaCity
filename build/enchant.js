...
enchant.Event = enchant.Class.create({
	...
	_initPosition: function(pageX, pageY, e) {
		...
		if(e) {
			...
			offsetX = Math.round(window.scrollX || window.pageXOffset + offsetX);
			offsetY = Math.round(window.scrollY || window.pageYOffset + offsetY);
		} else {
			offsetX = core._pageX;
			offsetY = core._pageY;
		}
		this.x = this.localX = (pageX - offsetX) / core.scale;
		this.y = this.localY = (pageY - offsetY) / core.scale;
	}
});
		...
		initialize: function(width, height) {
			...
			if (initial) {
				...
				stage.addEventListener('mousedown', function(e) {
					var core = enchant.Core.instance;
					var evt = new enchant.Event(enchant.Event.TOUCH_START);
					evt._initPosition(e.pageX, e.pageY, e);
